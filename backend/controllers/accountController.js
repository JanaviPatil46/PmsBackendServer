...
exports.getAccountById = async (req, res) => {
  try {
    ...
    const account = await Account.findById(accountId).populate({
        ...
        path: "adminUserId",
        select: "email"  // <-- SELECT EMAIL HERE
      });
...
exports.getAccountNamesByStatus = async (req, res) => {
  try {
    ...
    // Fetch accounts + populate contacts to get emails
    const accountlist = await Account.find(
      { active: isActive },
      "_id accountName contacts"
    )
      .populate({
        path: "contacts.contact",
    ...
    // Format response so each account includes array of email IDs
    const formatted = accountlist.map(acc => ({
      _id: acc._id,
    ...
    res.status(200).json({ success: true, accountlist: formatted });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};
...
// Get accounts by active status with their contact emails
exports.getAccountNamesWithEmails = async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, accountlist: result });

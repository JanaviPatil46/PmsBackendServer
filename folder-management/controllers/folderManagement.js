...
const createFolder = (req, res) => {
  try {
    ...
    // ? parent path logic same as account version
    let parentFolder;
    if (!parentPath) {
      // create inside templateId root
      if (!templateId)
        return res.status(400).json({ error: "templateId required" });

      parentFolder = path.join(BASE_UPLOAD_PATH, templateId);
    ...
    res.status(201).json({ message: "Template folder created", metaData });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: "Internal Server Error" });
  }
...
//const deleteItem = (req, res) => {
 ...
 //   } else {
     ...
     // const metaFile = path.join(path.dirname(fullPath), `${path.basename(fullPath)}.meta.json`)
//;
...
const applyTemplateToAccount = async (req, res) => {
  try {
    ...
    if (!templateId || !accountId) {
      return res.status(400).json({ 
        success: false, 
        error: "Template ID and Account ID are required" 
      });
    }

    // Define paths
    const templatePath = path.join(BASE_UPLOAD_PATH, templateId);
    const accountPath = path.join(__dirname, "../uploads/accounts", accountId);
    ...
    ensureDir(accountPath);

  

    // Copy ALL contents from template to account folder (not the template folder itself)
    ...
    for (const item of templateContents) {
      ...
      const destItemPath = path.join(accountPath, item);
      
      await fse.copy(sourceItemPath, destItemPath);
    }

    // Update metadata for all copied items
    const updateMetaRecursive = (dir) => {
      const entries = fs.readdirSync(dir);
      ...
      for (const entry of entries) {
        ...
        if (fs.lstatSync(fullPath).isDirectory()) {
          ...
          const folderMeta = readMeta(fullPath);
          folderMeta.appliedToAccount = accountId;
          folderMeta.appliedAt = new Date().toISOString();
          folderMeta.originalTemplate = templateId;
          folderMeta.path = path.relative(path.join(__dirname, "../uploads"), fullPath).replace(/\\/g, "/");
          writeMeta(fullPath, folderMeta);
          ...
          if (Object.keys(fileMeta).length) {
            fileMeta.appliedToAccount = accountId;
            fileMeta.appliedAt = new Date().toISOString();
            fileMeta.originalTemplate = templateId;
            fileMeta.path = path.relative(path.join(__dirname, "../uploads"), fullPath).replace(/\\/g, "/");
            writeMeta(parentDir, fileMeta, fileName);
          }
        }
      }
    ...
    updateMetaRecursive(accountPath);
    ...
    res.status(200).json({
      ...
      accountId,
      templateId,
      appliedPath: `accounts/${accountId}`,
      copiedItems: templateContents
    });
